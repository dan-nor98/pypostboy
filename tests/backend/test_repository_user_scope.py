...
def test_collection_update_and_reorder_require_user_owned_parent(sqlite_connection):
    ...
    with pytest.raises(ValueError, match="Parent collection not found"):
        Collections.update(
            collection["id"], user_one, {"parent_id": user_two_parent["id"]}
        )
...
def test_collection_duplicate_uses_user_scoped_original_children_and_requests(
    sqlite_connection,
):
    ...
    Requests.create(
        user_two, {"collection_id": other_root["id"], "name": "Other request"}
    )
    ...
    assert [request["name"] for request in copied_child["requests"]] == [
        "Child request"
    ]
    assert Collections.get_by_id(duplicate["id"], user_two) is None


def test_request_reorder_move_and_duplicate_require_user_owned_records(
    sqlite_connection,
):
    ...
    request_one = Requests.create(
        user_one, {"collection_id": collection_one["id"], "name": "One request"}
    )
    request_two = Requests.create(
        user_two, {"collection_id": collection_two["id"], "name": "Two request"}
    )
...
def test_request_instances_require_user_owned_request_and_instance(sqlite_connection):
    user_one = _create_user(sqlite_connection, "scope_instances_one")
    user_two = _create_user(sqlite_connection, "scope_instances_two")
    collection_one = Collections.create(user_one, {"name": "One"})
    collection_two = Collections.create(user_two, {"name": "Two"})
    request_one = Requests.create(
        user_one, {"collection_id": collection_one["id"], "name": "One request"}
    ...
    instance_one = RequestInstances.create(
        request_one["id"], user_one, {"name": "One snapshot"}
    )
    instance_two = RequestInstances.create(
        request_two["id"], user_two, {"name": "Two snapshot"}
    )

    with pytest.raises(ValueError, match="Request not found"):
        RequestInstances.get_by_request(request_two["id"], user_one)

    with pytest.raises(ValueError, match="Request not found"):
        RequestInstances.create(request_two["id"], user_one, {"name": "No access"})

    with pytest.raises(ValueError, match="Request instance not found"):
        RequestInstances.update(instance_two["id"], user_one, {"name": "No access"})

    assert RequestInstances.get_by_id(instance_two["id"], user_one) is None
    assert RequestInstances.delete(instance_two["id"], user_one) == {"deleted": 0}
    assert [
        item["id"]

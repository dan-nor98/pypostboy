...
class RequestInstances:
    ...
    def create(request_id, user_id=None, data=None):
        ...
        instance_id = insert_and_get_id(
            ...
            (
                ...
                data.get(
                    "body_content",
                    data.get("body_raw", request_obj.get("body_content", "")),
                ),
                data.get(
                    "body_raw_type",
                    request_obj.get("body_raw_type", "application/json"),
                ),
                safe_stringify(
                    data.get("form_data", request_obj.get("form_data", [])), "[]"
                ),
                data.get("auth_type", request_obj.get("auth_type", "none")),
                safe_stringify(
                    data.get("auth_data", request_obj.get("auth_data", {})), "{}"
                ),
        ...
        conn.commit()
        return RequestInstances.get_by_id(instance_id, user_id)
    ...
    def delete(id, user_id=None):
        ...
        db_execute(
            conn,
            "DELETE FROM request_instances WHERE id = ? AND user_id = ?",
            (id, user_id),
        )
        conn.commit()
        return {"deleted": 1}
